...
bool CModAppSystemGroup::Create()
{
...
#ifdef ANDROID
	if ( !IsServerOnly() && !Host_AllowLoadModule( "matchmaking", getenv("APP_LIB_PATH"), false ) )
#else
	if ( !IsServerOnly() && !Host_AllowLoadModule( "matchmaking", "GAMEBIN", false ) )
#endif
	{
	...
	g_pMatchmakingDllModule = g_pFileSystem->LoadModule(
		IsServerOnly() ? ( "matchmaking_ds" ) : ( "matchmaking" ),
#ifdef ANDROID
